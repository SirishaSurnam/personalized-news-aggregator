...
def home(request):
    ...
    if request.user.is_authenticated:
        user_categories = UserInterest.objects.filter(user=request.user).values_list('category', flat=True)
        if user_categories:
            articles = articles.filter(categories__in=user_categories).distinct()
    ...
    bookmarked_articles = []
    if request.user.is_authenticated:
        bookmarked_articles = list(Bookmark.objects.filter(user=request.user).values_list('article_id', flat=True))
...
class ArticleListAPI(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if self.request.user.is_authenticated:
            user_categories = UserInterest.objects.filter(user=self.request.user).values_list('category', flat=True)
            if user_categories:
